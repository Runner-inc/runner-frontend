...
function StartPage() {
    ...
    if (gameStarted && !vikingReachedBottom && !gameOver) {
      const startFalling = () => {
        const animate = () => {
          ...
          if (!vikingReachedBottom && !gameOver) animationFrameRef.current = requestAnimationFrame(animate);
        };
  ...
  const checkCollision = (vikingPos, skeletonList, jumping) => {
    ...
    return skeletonList.some(skel => {
      ...
      return vLeft < sLeft + collisionSize &&
    ...
    if (gameStarted && vikingReachedBottom && !gameOver) {
      ...
      const scheduleNextSpawn = () => {
        skeletonSpawnIntervalRef.current = setTimeout(() => {
          if (!gameOver) { spawnSkeleton(); scheduleNextSpawn(); }
      ...
      const animateSkeletons = () => {
        if (gameOver) return;
        setSkeletons(prev => {
          const updated = prev.map(skel => ({ ...skel, left: skel.left - skel.speed })).filter(s => s.left > -100);
  ...
  const handleStartGame = () => {
    setGameStarted(true); setGameOver(false); setVikingReachedBottom(false); setIsJumping(false);
    setVikingPosition({ top: -100, left: 0 }); setSkeletons([]); velocityRef.current = 0; setElapsedSeconds(0); scoreSubmittedRef.current = false;
  };
  ...
  const handlePageClick = () => {
    if (gameStarted && vikingReachedBottom && !gameOver) {
      ...
      setIsJumping(true);
      jumpTimeoutRef.current = setTimeout(() => { setIsJumping(false); if (vikingRef.current) vikingRef.current.classList.remove('viking-jumping'); jumpTimeoutRef.current = null; }, 800);
    }
  };
  ...
  return (
    ...
    >
      <div className="parallax-bg">
        <video className="bg-video" src="https://hwkaeeogqacgsfvbnfss.supabase.co/storage/v1/object/sign/viking_runner/grok-video-417d7c90-9f51-4e07-93c7-09cd5a9be976%20(online-video-cutter.com).mp4?token=eyJraWQiOiJzdG9yYWdlLXVybC1zaWduaW5nLWtleV8xNjk2MjFhNC04ZjgxLTRhMWItODNhZC0yMzRkNzBmODFjYWIiLCJhbGciOiJIUzI1NiJ9.eyJ1cmwiOiJ2aWtpbmdfcnVubmVyL2dyb2stdmlkZW8tNDE3ZDdjOTAtOWY1MS00ZTA3LTkzYzctMDljZDVhOWJlOTc2IChvbmxpbmUtdmlkZW8tY3V0dGVyLmNvbSkubXA0IiwiaWF0IjoxNzY1MDI5NDY4LCJleHAiOjc3NTY4Njk0Njh9.T_MMv3XFjv8dldEEf5U3Zh3XO-3nPKk0pXV8Z3MAmm4" autoPlay muted loop playsInline preload="auto" />
      </div>
      ...
      {!gameStarted ? (
        ...
        <>
          <div ref={vikingRef} className={`viking-animation-container ${vikingReachedBottom ? 'viking-running' : 'viking-falling'} ${isJumping ? 'viking-jumping' : ''}`} style={{ top: `${vikingPosition.top}px`, left: `${vikingPosition.left}px` }}>
            {!vikingReachedBottom ? (
              <AnimatedSprite images={start_viking} frameDuration={200} alt="Viking falling" width="75px" height="75px" />
            ) : (
              <AnimatedSprite images={isJumping ? viking_jump : viking_run} frameDuration={isJumping ? 120 : 150} alt={isJumping ? "Viking jumping" : "Viking running"} width="75px" height="75px" />
            )}
          </div>

          {skeletons.map(s => (
            <div key={s.id} className="skeleton-container" style={{ top: `${s.top}px`, left: `${s.left}px` }}>
              <AnimatedSprite images={skeleton} frameDuration={200} alt="Skeleton" width="75px" height="75px" />
            </div>
          ))}
